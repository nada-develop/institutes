...
$currentIndex = $data['teachers']->firstItem();
@endphp
...
<div class="row m-0">
    <div class="col-sm-12 col-md-5">
        <div class="dataTables_info" id="basic-datatable_info" role="status" aria-live="polite">
            @lang('dashboard.show')
            {{ $data['teachers']->currentPage() }} @lang('dashboard.from') {{ $data['teachers']->lastPage() }}

            @if ($data['teachers']->lastPage() > 1)
    ...
    <div class="col-sm-12 col-md-7 ">
        <div class="dataTables_paginate paging_simple_numbers" id="basic-datatable_paginate">
            {{ $data['teachers']->links('partials.paginator') }}
        </div>
    </div>
</div>
